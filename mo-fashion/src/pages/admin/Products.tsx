...
import { Plus, Search, Edit, Trash2, X, Package, Image as ImageIcon } from 'lucide-react';
...
export default function Products() {
  ...
  const handleOpenAdd = () => {
    ...
    setFormData({ 
      ...
      price: '', discount: '0', stock: '7', status: 'Active', images: [''] 
    });
    setIsModalOpen(true);
  };
  ...
  // 🚀 ১০০% গ্যারান্টিড সেভ বাটন হ্যান্ডলার
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (modalMode === 'add') {
        ...
        const newProdObj = { id: docRef.id, ...productData };
        updatedList = [newProdObj, ...products];
        toast.success("Product added live to database!");
      } else {
        const productRef = doc(db, "products", formData.id);
        await updateDoc(productRef, productData);
        updatedList = products.map(p => p.id === formData.id ? { id: formData.id, ...productData } : p);
        toast.success("Product updated live in database!");
      }
      ...
      toast.error(`Save Failed: ${error.message || "Check database rules"}`);
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="text-white pb-10">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Manage live database inventory</p>
        </div>
            ...
            <table className="w-full text-left whitespace-nowrap">
              <thead className="bg-[#111111] border-b border-[#D4AF37]/20">
                <tr>
                  ...
                  <th className="px-6 py-4 text-xs uppercase font-bold text-gray-300">Price</th>
                ...
                {filteredProducts.map((product: any) => {
                  ...
                  return (
                    <tr key={product.id} className="hover:bg-[#111111]/50">
                      ...
                      <td className="px-6 py-4 font-bold text-[#D4AF37]">${Number(product.price).toFixed(2)}</td>
            ...
            <form onSubmit={handleSubmit} className="overflow-y-auto custom-scrollbar p-6 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                  ...
                  <select value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})} className="w-full bg-[#111111] border border-gray-700 rounded-lg p-3 text-white focus:border-[#D4AF37] focus:outline-none">
                ...
                <div>
                  ...
                  <select value={formData.status} onChange={(e) => setFormData({...formData, status: e.target.value})} className="w-full bg-[#111111] border border-gray-700 rounded-lg p-3 text-white focus:border-[#D4AF37] focus:outline-none">
                    <option value="Active">Active</option><option value="Out of Stock">Out of Stock</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                ...
                <div>
                  <label className="block text-gray-300 text-sm mb-2 font-medium">Discount (%)</label>
                  <input type="number" value={formData.discount} onChange={(e) => setFormData({...formData, discount: e.target.value})} className="w-full bg-[#111111] border border-gray-700 rounded-lg p-3 text-white focus:border-[#D4AF37] focus:outline-none" />
                </div>
                <div>
